  ...
  return (
    <div className="max-w-3xl mx-auto my-8 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-6 space-y-6">
        {/* Quick Role Switcher Grid - ONLY visible to Admin */}
        {currentUser.role === 'admin' ? (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              {/* Resource Manager */}
              <div 
                onClick={() => handleSwitchAndProceed(SAMPLE_USERS[2], 'manager')}
                className={`p-3.5 rounded-xl border transition-all cursor-pointer hover:shadow-xs ${
              ...
              {/* Finance / Admin */}
              <div 
                onClick={() => handleSwitchAndProceed(SAMPLE_USERS[6], attemptedTab === 'database' ? 'database' : 'finance')}
                className={`p-3.5 rounded-xl border transition-all cursor-pointer hover:shadow-xs ${
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 flex items-start gap-3">
            ...
            <div className="text-xs text-slate-600 space-y-1">
              <p className="font-bold text-slate-800">Role Switching Policy</p>
              <p>
                Users with <strong className="text-slate-900">{currentUser.role === 'vendor' ? 'Vendor' : 'Manager'}</strong> profiles cannot change their identity to another role. Only authorized System Administrators have permission to switch roles and personas.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
          <button
            ...
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold shadow-xs transition-colors cursor-pointer"
          >
            <ArrowLeft className="w-4 h-4" />
            <span>Return to Permitted View ({currentConfig.label})</span>
          </button>

          {currentUser.role === 'admin' && suggestedUser && (
            <button
              id="rbac-switch-authorized-role-btn"
              onClick={() => handleSwitchAndProceed(suggestedUser, attemptedTab)}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-lg text-xs font-bold shadow-xs transition-colors cursor-pointer"
            >
              <UserCheck className="w-4 h-4" />
              <span>Admin: Continue as {suggestedUser.name} ({suggestedUser.role})</span>
            </button>
          )}
        </div>
      </div>
